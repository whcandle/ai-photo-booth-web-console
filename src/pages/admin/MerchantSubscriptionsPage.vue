<template>
  <div class="page">
    <div class="header">
      <h2>{{ t('page.merchantSubscriptions') }} - {{ merchantTitle }}</h2>
...
async function fetchData() {
  if (!merchantId.value || Number.isNaN(merchantId.value)) return
  loading.value = true
  try {
    const res = await listMerchantSubscriptions(merchantId.value, { pageNum: pageNum.value, pageSize: pageSize.value })
    if (res?.data?.success) {
...
async function submit() {
  ...
  try {
    if (editingId.value) {
      ...
      await createMerchantSubscription(merchantId.value, form)
    }
...
watch(
  ...
  async (id) => {
    ...
    pageNum.value = 1
    merchantTitle.value = `${t('merchant.merchantId')}: ${id}`
    try {
      const m = await getMerchant(id)
      if (m?.data?.success && m.data.data?.name) merchantTitle.value = m.data.data.name
    } catch (_) {}
    await fetchData()
  },
  { immediate: true }
